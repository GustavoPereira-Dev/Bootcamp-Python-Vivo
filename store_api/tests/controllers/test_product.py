...
import pytest
from tests.factories import product_data, product_fail_data
...
async def test_controller_create_should_not_return_success(client, products_url):
    response = await client.post(products_url, json=product_fail_data())

    response.json()

    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {
        "detail": "Preço está fora do limite esperado: (price > 5000 and price < 8000)"
    }
